...
exports.adminAnalytics = async (req, res) => {
  try {
    ...
    const [totalStudents, totalStaff, totalCourses, totalCertificates, activeToday] = await Promise.all([
    ...
    res.json({
      success: true,
      stats: { totalStudents, totalStaff, totalCourses, totalCertificates, activeToday, avgAttendance },
      recentEnrollments,
      topCourses
    });
  } catch (err) {
...
exports.studentAnalytics = async (req, res) => {
  try {
    ...
    const enrollments = await Enrollment.find({ studentId }).populate('courseId', 'title thumbnail category totalVideos');
