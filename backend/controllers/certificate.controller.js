...
const checkEligibility = async (studentId, courseId) => {
  ...
  if (enrollment.attendancePercentage < 80)
    return { eligible: false, reason: `Attendance ${enrollment.attendancePercentage.toFixed(1)}% is below 80%.` };

  // Rule 2: All videos watched
  const course = await Course.findById(courseId);
  if (enrollment.videosWatched.length < course.totalVideos)
    return { eligible: false, reason: `Only ${enrollment.videosWatched.length}/${course.totalVideos} videos watched.` };

  // Rule 3: All assignments completed
  const assignments = await Assignment.find({ courseId });
  for (const asgn of assignments) {
    const submission = asgn.submissions.find(s => s.studentId.toString() === studentId.toString());
    if (!submission || submission.status === 'submitted') {
      return { eligible: false, reason: 'Not all assignments are graded/completed.' };
    }
  }
...
exports.generateCertificate = async (req, res) => {
  try {
    ...
    if (existing) return res.json({ success: true, certificate: existing, message: 'Certificate already exists.' });
    ...
    const enrollment = await Enrollment.findOne({ studentId, courseId });
    const course = await Course.findById(courseId).select('staffId');
    ...
    // Enrollment completion is driven by progress/video completion;
...
exports.adminGenerateCertificate = async (req, res) => {
  try {
    ...
    if (attendancePercentage < 0 || attendancePercentage > 100) {
      return res.status(400).json({ success: false, message: 'Attendance percentage must be between 0-100' });
    }

    const existing = await Certificate.findOne({ studentId, courseId });
    if (existing) {
      return res.status(400).json({ success: false, message: 'Certificate already issued for this student-course combination' });
    }
