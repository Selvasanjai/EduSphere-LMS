...
exports.createLiveClass = async (req, res) => {
  try {
    const { courseId, title, description, scheduledStartTime, duration } = req.body;
...
exports.updateLiveClass = async (req, res) => {
  try {
    const { id } = req.params;
    const staffId = req.user._id;
    const { title, description, chatEnabled, raiseHandEnabled, screenShareEnabled, recordingEnabled } = req.body;
    ...
    if (raiseHandEnabled !== undefined) liveClass.raiseHandEnabled = raiseHandEnabled;
    if (screenShareEnabled !== undefined) liveClass.screenShareEnabled = screenShareEnabled;
    if (recordingEnabled !== undefined) liveClass.recordingEnabled = recordingEnabled;
...
exports.raiseHand = async (req, res) => {
  try {
    ...
    const activeHand = liveClass.raisedHands.find(
      hand => hand.studentId.toString() === studentId.toString() && !hand.isAnswered
    );
