...
router.post('/:id/submit', protect, authorize('student'), async (req, res) => {
  try {
    ...
    const alreadySubmitted = assignment.submissions.find(s => s.studentId.toString() === req.user._id.toString());
    if (alreadySubmitted) return res.status(400).json({ success: false, message: 'Already submitted.' });
    assignment.submissions.push({ studentId: req.user._id, fileUrl: req.body.fileUrl });
...
router.patch('/:id/grade/:studentId', protect, authorize('staff', 'admin'), async (req, res) => {
  try {
    ...
    if (req.user.role === 'staff' && assignment.staffId.toString() !== req.user._id.toString()) {
      return res.status(403).json({ success: false, message: 'You can only grade assignments in your own courses.' });
    }
