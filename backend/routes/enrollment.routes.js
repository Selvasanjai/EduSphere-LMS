...
router.post('/', protect, authorize('student'), async (req, res) => {
  try {
    ...
    const course = await Course.findById(courseId).select(
      'isPublished isApproved enrollmentCount maxStudents'
    );
    if (!course)
      return res
        .status(404)
    ...
    if (activeCount >= 2) {
      return res.status(400).json({
        success: false,
        message:
          'You can enroll in only 2 courses at a time. Complete one course to enroll in a new one.',
      });
    }
    ...
    if ((course.enrollmentCount || 0) >= max) {
      return res
        ...
        .json({
          success: false,
          message: `Enrollment limit reached (max ${max} students).`,
        });
    }
...
router.post(
  '/course/:courseId/videos/:videoId/complete',
  protect,
  authorize('student'),
  async (req, res) => {
    try {
      ...
      const videoExists =
        ...
        course.videos.some((v) => v._id.toString() === videoId);
      if (!videoExists)
        return res
          .status(404)
          .json({ success: false, message: 'Video not found in this course.' });

      const alreadyWatched = enrollment.videosWatched?.some(
        (v) => v.toString() === videoId
      );
      if (!alreadyWatched) {
        enrollment.videosWatched = Array.isArray(enrollment.videosWatched)
      ...
      const watchedCount = new Set(
        (enrollment.videosWatched || []).map((v) => v.toString())
      ).size;
      const progress =
        total > 0 ? Math.min(100, Math.round((watchedCount / total) * 100)) : 0;
      ...
      if (progress >= 100) {
        ...
        if (!certificate) {
          // Calculate score and grade (basic logic matching certificate.controller.js)
          const score = Math.min(100, Math.round(progress * 0.7 + enrollment.attendancePercentage * 0.3));
          let grade = 'F', distinctionStatus = '';
          if (score >= 90) { grade = 'A+'; distinctionStatus = 'with Distinction'; }
          else if (score >= 80) { grade = 'A'; distinctionStatus = 'with Merit'; }
