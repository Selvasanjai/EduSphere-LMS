...
import { FaTachometerAlt, FaBook, FaUsers, FaChartBar, FaCertificate, FaCog, FaSignOutAlt, FaClipboardCheck, FaVideo, FaUserGraduate, FaUser } from 'react-icons/fa';
...
const adminNav = [
  ...
  { icon: <FaClipboardCheck />, label: 'Attendance', path: '/admin/attendance' },
];
...
export default function Sidebar() {
  ...
  return (
    <nav className="sidebar">
      ...
      <button className="nav-item" onClick={() => navigate(`/${user?.role}/profile`)} style={{ justifyContent: 'flex-start' }}>
        <FaUser /> My Profile
      </button>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', marginBottom: 8 }}>
        <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'linear-gradient(135deg, var(--accent-cyan), var(--accent-violet))', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: 14, color: '#0a0f1e' }}>
      ...
      <button className="nav-item" onClick={() => { logout(); navigate('/login'); }} style={{ color: 'var(--accent-rose)' }}>
        <FaSignOutAlt /> Logout
      </button>
    </nav>
  );
