...
const StudentLiveClassesList = ({ courseId }) => {
  ...
  const upcomingLiveClasses = liveClasses.filter(lc => lc.status === 'scheduled');
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '1rem' }}>
            {upcomingLiveClasses.map((lc) => (
              ...
              >
                ...
                {lc.description && (
                  <p style={{ color: '#666', fontSize: '0.9rem', marginBottom: '1rem', margin: '0.5rem 0' }}>
                    {lc.description}
                  </p>
                )}
                ...
                <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginBottom: '1rem' }}>
                  ...
                  {lc.raiseHandEnabled && <span style={{ background: '#e7f3ff', color: '#0056b3', padding: '0.25rem 0.75rem', borderRadius: '20px', fontSize: '0.75rem' }}>✋ Q&A</span>}
                  {lc.screenShareEnabled && <span style={{ background: '#e7f3ff', color: '#0056b3', padding: '0.25rem 0.75rem', borderRadius: '20px', fontSize: '0.75rem' }}>🖥️ Share</span>}
