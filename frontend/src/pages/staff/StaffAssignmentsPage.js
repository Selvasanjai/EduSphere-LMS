...
export default function StaffAssignmentsPage() {
  ...
  const fetchAssignments = async (courseId) => {
    try {
      const { data } = await axios.get(`${API_BASE_URL}/assignments/course/${courseId}`);
        ...
        <>
          ...
          {assignments.length === 0 ? (
            <div className="card" style={{ textAlign: 'center', padding: '40px' }}>
              <p style={{ color: 'var(--text-muted)', marginBottom: 20 }}>No assignments yet</p>
              <button className="btn btn-primary" onClick={() => setShowModal(true)}>
              ...
              {assignments.map(assignment => (
                <div key={assignment._id} className="card">
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: 12 }}>
                    <div>
                      <h3 style={{ fontFamily: 'var(--font-display)', marginBottom: 4 }}>{assignment.title}</h3>
                      <p style={{ fontSize: 13, color: 'var(--text-muted)' }}>
                        {assignment.description.substring(0, 100)}...
                      </p>
                    </div>
                    <span className="badge badge-cyan">Max: {assignment.maxMarks} marks</span>
                  </div>
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center', fontSize: 13, color: 'var(--text-muted)', marginBottom: 16 }}>
