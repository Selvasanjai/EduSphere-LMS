...
const StaffLiveClassPage = () => {
  ...
  const handleCreateLiveClass = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        await axios.patch(`${API_URL}/live-classes/${editingId}`, payload, {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
        });
        toast.success('Live class updated successfully');
      } else {
        await axios.post(`${API_URL}/live-classes`, payload, {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
        });
  ...
  const handleDeleteLiveClass = async (classId) => {
    ...
    try {
      await axios.delete(`${API_URL}/live-classes/${classId}`, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
      });
  ...
  const handleEditLiveClass = (liveClass) => {
    ...
    setFormData({
      ...
      scheduledStartTime: new Date(liveClass.scheduledStartTime)
        .toISOString()
        .slice(0, 16),
      ...
      {showCreateForm && (
        ...
        >
          <h2 style={{ marginTop: 0 }}>
            {editingId ? 'Edit' : 'Schedule'} Live Class
          </h2>
